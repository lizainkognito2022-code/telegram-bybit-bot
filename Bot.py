...
def analyze(c4h, c1h, c30):
    ...
    higher_lows = check_higher_lows(c1h)
...
def create_chart(candles, resistance):
    """
    Создаёт PNG со свечным графиком.
    Фон: чёрный.
    Рост: зелёный.
    Падение: красный.
    Показываются последние 80 свечей.
    """
    ...
    for index, candle in enumerate(candles):
        ...
        # Если тело практически нулевое,
        # всё равно делаем его видимым.
        if body_height == 0:
            body_height = max(
                (high - low) * 0.01,
                close_price * 0.0001
    ...
    # чтобы не создавать обязательный
    # постоянный файл на диске.
...
def main():
    ...
    # Отправляем график в тот же Telegram-чат.
    send_photo(
        chart,
        "BTC/USD 1H"
    )
