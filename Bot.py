...
def get_volume_ratio(candles, lookback=20):
    ...
    previous_candles = candles[
        max(0, len(candles) - lookback - 1):-1
    ]
...
def setup_a(candles):
    ...
    range1 = (
        max(c["high"] for c in block1)
        - min(c["low"] for c in block1)
    )

    range2 = (
        max(c["high"] for c in block2)
        - min(c["low"] for c in block2)
    )

    range3 = (
        max(c["high"] for c in block3)
        - min(c["low"] for c in block3)
    )
    ...
    distance1 = resistance - close1
    distance2 = resistance - close2
    distance3 = resistance - close3

    approach = (
        distance2 < distance1
        and distance3 < distance2
    )
...
def get_open_interest():
    ...
    if not numbers:
        raise Exception(
            "Kraken OI: числовые данные не найдены"
        )
...
def get_funding():
    ...
    if not numbers:
        raise Exception(
            "Kraken Funding: числовые данные не найдены"
        )
...
def market_analysis():
    ...
    try:
        funding = get_funding()
        funding_status = interpret_funding(
            funding
        )
        funding_error = None
    ...
    text = (
        ...
        "━━━━━━━━━━━━━━\n"
        f"Статус: "
        f"{'🟢 YES' if a['signal'] else '⚪ NO'}\n"
        f"Resistance: {a['resistance']:,.2f}\n"
        f"Distance: {a['distance']:.2f}%\n"
        f"Higher Lows: {a['higher_lows']}/9\n"
        f"Compression: "
        f"{'YES' if a['compression'] else 'NO'}\n"
        f"Approach: "
        f"{'YES' if a['approach'] else 'NO'}\n"
        ...
        "━━━━━━━━━━━━━━\n"
        f"Статус: "
        f"{'🟢 YES' if b['signal'] else '⚪ NO'}\n"
        f"Resistance: {b['resistance']:,.2f}\n"
        f"Breakout: "
        f"{'YES' if b['breakout'] else 'NO'}\n"
        f"Retest: "
        f"{'YES' if b['retest'] else 'NO'}\n"
        f"Hold: "
        f"{'YES' if b['hold'] else 'NO'}\n"
        "\n"
        "━━━━━━━━━━━━━━\n"
        "SETUP C\n"
        "━━━━━━━━━━━━━━\n"
        f"Статус: "
        f"{'🟢 YES' if c['signal'] else '⚪ NO'}\n"
        f"Liquidity Low: "
        f"{c['liquidity_low']:,.2f}\n"
        f"Sweep: "
        f"{'YES' if c['sweep'] else 'NO'}\n"
        f"Reclaim: "
        f"{'YES' if c['reclaim'] else 'NO'}\n"
        f"Structure Shift: "
        f"{'YES' if c['structure_shift'] else 'NO'}\n"
        "\n"
        "━━━━━━━━━━━━━━\n"
        "30M CONFIRMATION\n"
        "━━━━━━━━━━━━━━\n"
        f"Structure: "
        f"{confirmation_30m['structure']}\n"
        f"Volume: "
        f"{confirmation_30m['volume_ratio']:.2f}x\n"
